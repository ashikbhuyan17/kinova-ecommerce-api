...
- `GET /api/v1/category/{id}` - Get category by ID (Public)
- `GET /api/v1/category/slug/{slug}` - Get category by slug (Public)
...
Categories are top-level product categories with the following fields:

- `_id` - MongoDB ObjectId (Category ID)
- `slug` - URL-friendly slug (auto-generated from name if not provided)
- `name` - Category name (required, max 200 characters)
- `image` - Base64 encoded image (optional, max 1MB)
- `description` - Category description (optional)
- `createdAt` - Creation timestamp
- `updatedAt` - Last update timestamp

**Slug Generation**: Slugs are automatically generated from the category name using `slugify` library. They are lowercase, URL-friendly, and unique across all categories.
...
**Slug Uniqueness**: SubCategory slugs must be unique within the same category. The same slug can exist in different categories.

**Example**:
- Category: "Electronics" (slug: "electronics")
  - SubCategory: "Smartphones" (slug: "smartphones")
  - SubCategory: "Laptops" (slug: "laptops")
- Category: "Clothing" (slug: "clothing")
  - SubCategory: "Smartphones" (slug: "smartphones") ✅ Allowed (different category)
...
Blogs, Categories, and SubCategories support base64 encoded images:
...
const swaggerUiOptions = {
  ...
  customSiteTitle: 'Kinova E-commerce API Documentation',
